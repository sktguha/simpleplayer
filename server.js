...
// ------------------------------------------------------------
// /eval : execute arbitrary JS server-side and return the result.
//
// Deliberately added despite the obvious security issue, per request:
// this is for a trusted local/tailscale-only network, and the point is to
// avoid writing a bespoke API route every time you want to peek at or
// tweak server state. DO NOT expose this port publicly.
//
// Runs in a vm context that closes over the module's live state (latest,
// downloaded, dead, pending, downloading, segmentMs, etc) so you can both
// read and mutate them, e.g.:
//   latest
//   Array.from(downloaded).slice(-5)
//   segmentMs = 1500
// Wrapped in vm.runInNewContext with a `require` shim so it can still
// reach node builtins if genuinely needed, but the main state vars are
// pulled in directly for convenience.
// ------------------------------------------------------------
function handleEval(code) {
  ...
  const script = new vm.Script(`result = (function(){ return (${code}\n); })();`);
  script.runInContext(sandbox, { timeout: 2000 });

  // writeback for the primitives that are meaningful to mutate live
  if (typeof sandbox.segmentMs === "number") segmentMs = sandbox.segmentMs;
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && pathname === "/state") {
    // Every id we've ever heard of (attempted, downloading, pending, or dead)
    // gets a status. Previously `pending` and `downloading` ids were left out
    // entirely, which made the client see them as neither ready nor dead and
    // just spin forever until the server happened to backfill them.
    const ids = new Set([...downloaded, ...dead, ...pending, ...downloading]);
    const segments = Array.from(ids)
      .sort((a, b) => a - b)
      .map(id => {
        let status;
        if (downloaded.has(id)) status = "ready";
        else if (downloading.has(id)) status = "downloading";
        else if (pending.has(id)) status = "pending";
        else status = "dead";
        return { id, status };
      });
  ...
  if (req.method === "POST" && pathname === "/eval") {
    ...
    req.on("end", () => {
      ...
      try {
        ...
        res.end(JSON.stringify({ error: "invalid JSON body, expected { code: string }" }));
        return;
      }
      try {
        const result = handleEval(code);
        res.writeHead(200, { "Content-Type": "application/json" });
        // result may contain Sets etc that JSON.stringify can't handle directly;
        // fall back to a safe stringification if plain stringify fails.
        let payload;
        try {
          payload = JSON.stringify({ result });
        } catch {
          payload = JSON.stringify({ result: String(result) });
        }
        res.end(payload);
      } catch (e) {
        res.writeHead(200, { "Content-Type": "application/json" });
